...
* Value type is <<string,string>>
* Allowed values are: `TLSv1.1`, `TLSv1.2`, 'TLSv1.3', 'SSLv3', 'TLSv1'
* There is no default value for this setting

List of allowed SSL/TLS versions to use when establishing a connection to the HTTP endpoint.
The default depends on the JDK being used and with up-to-date Logstash/Java is usually `['TLSv1.2', 'TLSv1.3']`.
When using Java 8 `TLSv1.3` is only supported since **8u262** (AdoptOpenJDK).
The `SSLv3` and `TLSv1` protocols are insecure and are provided when connecting to legacy systems.
