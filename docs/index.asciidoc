...
Allows for disabling the verification performed by the plugin when the TLS certificate
is retrieved from the server. The certificate's host-name must match the `url`'s host.
Also, set the option to `none` when dealing with self signed certificates as those aren't
considered valid, unless the certificate is part of the `truststore`.
